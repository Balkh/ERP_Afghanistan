...
class SimpleModelTests(TestCase):
    ...
    def test_batch_model(self):
        ...
        batch = Batch.objects.create(product=prod, batch_number='B001', quantity=Decimal('100'), remaining_quantity=Decimal('100'), manufacturing_date=(timezone.now() - timedelta(days=30)).date(), expiry_date=date.today() + timedelta(days=365), purchase_price=Decimal('10'), sale_price=Decimal('15'), location='A1')
