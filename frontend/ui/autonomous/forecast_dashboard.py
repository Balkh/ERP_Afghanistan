...
class _ForecastCard(QFrame):
    def __init__(self, domain: str, metric: str, current: float, predicted: float,
        ...
        title.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY}; font-size: 14px; font-weight: bold;")
        ...
        curr.setStyleSheet(f"color: {COLOR_TEXT_SECONDARY}; font-size: 12px;")
        val_row.addWidget(curr)

        pred = QLabel(f"Predicted: {predicted:.2f}")
        pred.setStyleSheet(f"color: {color}; font-size: 12px; font-weight: bold;")
        val_row.addWidget(pred)

        dr = QLabel(f"{arrow.get(direction, '?')} {direction}")
        dr.setStyleSheet(f"color: {color}; font-size: 12px; font-weight: bold;")
        val_row.addWidget(dr)
        val_row.addStretch()
        layout.addLayout(val_row)

        ci = QLabel(f"95% CI: [{low:.2f}, {high:.2f}]")
        ci.setStyleSheet(f"color: {COLOR_TEXT_MUTED}; font-size: 10px;")
        layout.addWidget(ci)
