...
class CustomerPaymentWorkspace(BaseScreen):
    """Customer payment workspace - unified view for customer payment operations."""

    payment_processed = Signal(dict)
    allocation_completed = Signal(dict)

    def __init__(self, parent=None, api_client=None):
        super().__init__(parent, screen_id="customer_payments")
    ...
    def _on_screen_shown(self):
        """Prevent BaseScreen from auto-loading on show — we load on customer selection."""
    ...
    def setup_ui(self):
        ...
        self.kpi_customer_balance = MiniMetricCard("Customer Balance", "0.00", COLOR_DANGER)
        self.kpi_credit_limit = MiniMetricCard("Credit Limit", "0.00", COLOR_PRIMARY)
        ...
        self.kpi_layout.addWidget(self.kpi_customer_balance, 0, 0)
        self.kpi_layout.addWidget(self.kpi_credit_limit, 0, 1)
        self.kpi_layout.addWidget(self.kpi_outstanding, 0, 2)
        self.kpi_layout.addWidget(self.kpi_unallocated, 0, 3)
    ...
    def _create_invoices_section(self):
        ...
        columns = [
            ...
            TableColumn("days_overdue", "Days Overdue", width=100, align="center"),
        ]
    ...
    def _create_allocation_summary(self):
        ...
        self.lbl_available_credit = QLabel("Available Credit: 0.00")
    ...
    def _update_workspace(self, data):
        ...
        balance = customer.get("derived_balance", {})
        self.kpi_customer_balance.update_value(f"{self._safe_float(balance.get('balance', 0)):,.2f}")
        self.kpi_credit_limit.update_value(f"{self._safe_float(customer.get('credit_limit', 0)):,.2f}")
        ...
        available = self._safe_float(customer.get("credit_limit", 0)) - self._safe_float(balance.get("balance", 0))
        self.lbl_available_credit.setText(f"Available Credit: {available:,.2f}")

    def _update_invoices_table(self):
        """Update invoices table."""
        data = []
        for inv in self.outstanding_invoices:
            remaining = self._safe_float(inv.get("remaining", 0))
            ...
            data.append({
                ...
                "days_overdue": str(days_overdue) if days_overdue > 0 else "-",
            })
    ...
    def refresh_workspace(self):
        ...
        if self.customer_id:
            self.load_workspace(self.customer_id)
