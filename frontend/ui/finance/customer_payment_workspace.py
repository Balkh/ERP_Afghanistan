...
class CustomerPaymentWorkspace(BaseScreen):
    ...
    def _update_workspace(self, data):
        ...
        self.kpi_customer_balance.update_value(f"{safe_float(balance.get('balance', 0)):,.2f}")
        self.kpi_credit_limit.update_value(f"{safe_float(customer.get('credit_limit', 0)):,.2f}")
        ...
        available = safe_float(customer.get("credit_limit", 0)) - safe_float(balance.get("balance", 0))
    ...
    def _update_invoices_table(self):
        ...
        for inv in self.outstanding_invoices:
            remaining = safe_float(inv.get("remaining", 0))
