"""
Enhanced Barcode Scanner Widget — USB HID scanner support with timing detection.
Handles keyboard wedge scanners, USB barcode scanners, and manual entry.
"""
...
class BarcodeScannerInput(QLineEdit):
    """
    QLineEdit with USB barcode scanner detection via timing analysis.
    
    USB scanners type characters extremely fast (<20ms between keystrokes),
    while human typing is much slower (>50ms). This widget uses that difference
    to distinguish between scanner input and manual typing.
    """
...
class BarcodeSearchResultsDropdown(QFrame):
    """Dropdown frame showing barcode search results."""
